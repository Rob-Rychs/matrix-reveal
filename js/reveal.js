 ...
 */
  ...
  let SLIDES_SELECTOR = '.slides section',
  ...
  let images = toArray(document.getElementsByTagName('img')),
  ...
  let rgb = colorToRgb(computedBackgroundStyle.backgroundColor);
  ...
  let numberOfSlides = dom.wrapper.querySelectorAll(SLIDES_SELECTOR).length;
...
oldHeight = element.style.height;
  ...
  let anchors = dom.wrapper.querySelectorAll(`${SLIDES_SELECTOR  } a`);
  ...
  let anchors = dom.wrapper.querySelectorAll(`${SLIDES_SELECTOR  } a.roll`);
  ...
  let anchors = toArray(document.querySelectorAll(selector || 'a'));
  ...
  let slides = toArray(dom.wrapper.querySelectorAll(SLIDES_SELECTOR));
  ...
  let margin = 70;
  let slideSize = getComputedSlideSize();
  ...
  function pause() {
  if (config.pause) {
  let wasPaused = dom.wrapper.classList.contains('paused');
  ...
  if (dom.wrapper.querySelector(HOME_SLIDE_SELECTOR).classList.contains('present')) {
				// Launch async task
  setTimeout(() => {
  let slides = toArray(dom.wrapper.querySelectorAll(`${HORIZONTAL_SLIDES_SELECTOR  }.stack`)), 
i;
  for (i in slides) {
  if (slides[i]) {
							// Reset stack
  ...
  if (verticalSlides.length === 0) sortFragments(horizontalSlide.querySelectorAll('.fragment'));
	 ...
	 */
  function shuffle() {
  let slides = toArray(dom.wrapper.querySelectorAll(HORIZONTAL_SLIDES_SELECTOR));
  ...
  if (config.fragments) {
  let pastFragments = toArray(element.querySelectorAll('.fragment'));

						// Show all fragments on prior slides
  while (pastFragments.length) {
  let pastFragment = pastFragments.pop();
  ...
  if (config.fragments) {
  let futureFragments = toArray(element.querySelectorAll('.fragment.visible'));

						// No fragments in future slides should be visible ahead of time
  while (futureFragments.length) {
  let futureFragment = futureFragments.pop();
  ...
  let backgroundImageURL = currentBackground.style.backgroundImage || '';
...
backgroundHeight;
  ...
  let _appendParamToIframeSource = function (sourceAttribute, sourceURL, param) {
  ...
  let horizontalSlide = dom.wrapper.querySelectorAll(HORIZONTAL_SLIDES_SELECTOR)[x];
  ...
  let horizontalBackground = dom.wrapper.querySelectorAll('.backgrounds>.slide-background')[x];
  let verticalBackgrounds = horizontalBackground && horizontalBackground.querySelectorAll('.slide-background');
  ...
  let currentSpan = distanceBetween({
  x: event.touches[1].clientX,
  y: event.touches[1].clientY,
}, {
  x: touch.startX,
  ...
  let slidesTotal = toArray(dom.wrapper.querySelectorAll(HORIZONTAL_SLIDES_SELECTOR)).length;
  ...
  let isHidden = document.webkitHidden ||
						document.msHidden ||
						document.hidden;
